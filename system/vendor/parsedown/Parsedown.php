...
class Parsedown
{
    ...
    protected function inlineCode($Excerpt)
    {
        ...
        // To support `\`\`` syntax
        $re = "/^([$marker]++)[ ]*+(.+?)[ ]*+(?<![^\\\\][\\\\$marker])\\1(?!$marker)/s";

        // Original: if (preg_match('/^([' . $marker . ']++)[ ]*+(.+?)[ ]*+(?<![' . $marker . '])\1(?!' . $marker . ')/s', $Excerpt['text'], $matches)) {
    ...
    protected function inlineUrl($Excerpt)
    {
        ...
        if (
            ...
            // Some example URLs have '{var}' in URL so we don't want to link those.
            if (!strpos($url, '{') !== false) {
    ...
    protected function elements(array $Elements)
    {
        ...
        foreach ($Elements as $Element) {
            ...
            $markup .= ($autoBreak ? "" : '') . $this->element($Element);
