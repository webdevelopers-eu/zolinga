...
The MCP `tools/list` JSON-RPC method. Dispatched by the [MCP gateway](:Zolinga Core:Running the System:MCP) as a [`Tools\ListEvent`](:Zolinga Core:Events and Listeners:MCP) with the `mcp` origin. The event type is `mcp:tools/list`.
