...
| Property     | Type                              | Notes |
|--------------|-----------------------------------|-------|
| `type`       | `string`                          | The Zolinga event type. Protocol methods use the method with `/` → `:` and an `mcp:` prefix (e.g. `tools/list` → `mcp:tools/list`), while `tools/call` uses the bare tool name (`params.name`). Each concrete subclass sets the base type in its constructor. |
...
For `tools/call` invocations the gateway dispatches a `Tools\CallEvent` with `type = "<name>"`. The gateway always wraps the handler's response in the MCP `{ content, isError, structuredContent }` envelope, never in a JSON-RPC `error` block.
