...
- **MCP (Model Context Protocol)**
    > The fourth way to run Zolinga is to expose its events as tools to an LLM-powered MCP client. The JSON-RPC 2.0 endpoint at `/mcp` is a thin gateway that dispatches every method as a Zolinga `Mcp\Event` subclass and serializes the response back as a JSON-RPC 2.0 message. See [MCP](:Zolinga Core:Running the System:MCP) for the full guide.
- **Custom Scripts**
